...
def dibuja_cajas_3d(contenedor, caja_dim, distribucion):
    ...
    # Ajuste realista de aspecto según las dimensiones reales
...
def main():
    ...
    caja_peso = st.number_input("Weight per box (kg)", min_value=0.01, value=15.0, format="%.2f")
    ...
    if st.button("Calculate"):
        ...
        st.write(f"Distribution (UCM): Length x Width x Height = {distribucion}")

        # Cálculo de peso y volumen
        peso_total = total * caja_peso

        volumen_caja = (rotacion[0] / 1000) * (rotacion[1] / 1000) * (rotacion[2] / 1000)
        volumen_total = volumen_caja * total

        volumen_contenedor = (contenedor_dim[0] / 1000) * (contenedor_dim[1] / 1000) * (contenedor_dim[2] / 1000)

        porcentaje_volumen = volumen_total / volumen_contenedor * 100

        peso_max_contenedor = 28000  # kg
        porcentaje_peso = peso_total / peso_max_contenedor * 100

        st.write(f"Total weight (kg): **{peso_total:,.2f}**")
        st.write(f"Weight saturation (%): **{porcentaje_peso:.2f}%**")
        st.write(f"Volume saturation (%): **{porcentaje_volumen:.2f}%**")
...
#python -m streamlit run Container3D_2.py